...
router.post("/updateUser", async (req, res) => {
  try {
    ...
    if (score !== undefined && user.aptitudePassingMarks !== undefined) {
      if (score >= user.aptitudePassingMarks) {
        if (!user.aptitudePassesCandidates.includes(userEmail)) {
    ...
    if (technicalScore !== undefined && user.technicalPassingMarks !== undefined) {
      if (technicalScore >= user.technicalPassingMarks) {
        if (!user.techPassesCandidates.includes(userEmail)) {
    ...
    // Update email if provided and check for duplication
    if (email) {
      const emailExists = await User.findOne({ email });
      if (emailExists && emailExists._id.toString() !== userId) {
