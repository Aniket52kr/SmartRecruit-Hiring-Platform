...
// Global flags (consider refactoring into context or state if needed)
let currentlyScored = 0;
let isPasteAllowed = true;
...
const TechRound = () => {
  ...
  // Fetch problems (only after login)
  useEffect(() => {
  ...
  const handleTimeExpired = async () => {
    setIsTimerRunning(false);
    alert("Technical round time has expired!");
    handleEndSession(); // Auto-submit on timeout
  };
