...
const QuizComponent = () => {
  ...
  // update handle quiz submit function:-
  const handleQuizSubmit = async () => {
    const userId = userid;
    const userEmail = email;
    setemail(email);
    ...
    try {
      ...
      if (score >= passingMarks) {
        ...
        console.log("Sending rejection email...", templateParams);
      ...
      console.log(`Quiz completed! Your score: ${score}`);
      alert(
        "You have successfully completed the Aptitude round, we will update you through email soon."
      );
      setSubmitted(false);
    } catch (error) {
      console.error("Error submitting quiz:", error);
